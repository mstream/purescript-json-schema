...
type GetInputResponse =
  { filesystem ∷ Map String String, command ∷ Command }

data Query a = GetInput (GetInputResponse → a)
...
handleAction = case _ of
  ...
  RunProgram → do
    { filesystem, command } ← getOptionsFormInput
    programOutput ← runCommand filesystem case command of
    ...
    put { programOutput: Just programOutput }
  where
  getOptionsFormInput ∷ ComponentMonad m GetInputResponse
  getOptionsFormInput = do
    response ← H.request (Proxy ∷ Proxy "optionsForm") unit GetInput
    maybe
      (throwError $ Exception.error "Failed to read options form data")
      pure
      response
  ...
  runCommand
    ∷ Map String String
    → AppM ProgramOutput
    → ComponentMonad m ProgramOutput
