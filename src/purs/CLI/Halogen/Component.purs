...
commandFormComponent
  ∷ ∀ m. Component Sandbox.Query Unit Sandbox.Output m
...
handleQuery ∷ ∀ a m. Sandbox.Query a → ComponentMonad m (Maybe a)
handleQuery = case _ of
  Sandbox.GetInput reply → do
    state ← get
        ...
        case state.selectedCommand of
          CompatCommand → do
            input ← H.request
              ...
              unit
              Command.GetInput
            pure $ input <#> \{ filesystem, options } →
              reply { command: Compat format options, filesystem }
          DiffCommand → do
            input ← H.request
              ...
              Command.GetInput
            pure $ input <#> \{ filesystem, options } →
              reply { command: Diff format options, filesystem }
          ValidateCommand → do
            input ← H.request
              ...
              Command.GetInput
            pure $ input <#> \{ filesystem, options } →
              reply { command: Validate format options, filesystem }
