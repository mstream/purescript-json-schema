...
encloseSep left right sep' ds = case ds of
  ...
  _ → align
    ( cat
        ( LL.toUnfoldable $ LL.zipWith (<>) (left LL.: LL.repeat sep')
            (LL.fromFoldable ds)
        ) <> right
    )
...
punctuate p arr =
  ...
  in
    Array.mapWithIndex (\idx d → if lastIdx == idx then d else d <> p)
      arr
...
foldr1 ∷ ∀ a. Monoid a ⇒ (a → a → a) → Array a → a
...
renderFits
  ∷ (Int → Int → Int → LazySimpleDoc → Boolean)
  → Number
  → Int
  → Doc
  → SimpleDoc
renderFits fits rfrac w headNode
  ...
  nicest' n k i ds x y =
    let
      ...
      width' = min (w - k) (r - k + n)
    in
      if fits w (min n k) width' x' then x'
      else let y' = best n k (Cons i y ds) in y'
...
displayS SFail = unsafeCrashWith $
