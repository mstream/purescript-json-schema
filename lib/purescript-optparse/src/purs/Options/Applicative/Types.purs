...
instance isCmdStartShow ∷ Show IsCmdStart where
  show = genericShow
...
instance backtrackingShow ∷ Show Backtracking where
  show = genericShow
...
instance parserPrefsShow ∷ Show ParserPrefs where
  show = genericShow
...
instance optNameShow ∷ Show OptName where
  show = genericShow
...
instance optVisibilityShow ∷ Show OptVisibility where
  show = genericShow
...
newtype OptProperties = OptProperties
  ...
  , propShowDefault ∷
      Maybe String -- ^ what to show in the help text as the default
  , propDescMod ∷
      Maybe (Doc → Doc) -- ^ a function to run over the brief description
  }

derive instance optPropertiesNewtype ∷ Newtype OptProperties _
instance optPropertiesShow ∷ Show OptProperties where
  show (OptProperties r) = "(OptProperties "
    <> show
...
instance optionShow ∷ Show (Option a) where
  show (Option opt) = "(Option "
    <> show { optProps: opt.optProps, optMain: "<OptReader>" }
    <> ")"
...
readerAbort ∷ ∀ a. ParseError → ReadM a
readerAbort = ReadM <<< lift <<< throwError

-- | Abort option reader by exiting with an error message.
readerError ∷ ∀ a. String → ReadM a
...
data OptReader a
  ...
  | ArgReader (CReader a)
  -- ^ argument reader
  | CmdReader (Maybe String) (Array String)
...
instance parserFunctor ∷ Functor Parser where
  ...
  map f (MultP e) = runExists
    (\(MultPE p1 p2) → MultP $ mkExists $ MultPE (map (f <<< _) p1) p2)
    e
...
fromM ∷ ∀ a. ParserM a → Parser a
fromM (ParserM f) = BindP f

oneM ∷ ∀ a. Parser a → ParserM a
oneM = liftF >>> ParserM

manyM ∷ ∀ a. Parser a → ParserM (List a)
...
someM ∷ ∀ a. Parser a → ParserM (NonEmptyList a)
...
some ∷ ∀ a. Parser a → Parser (NonEmptyList a)
some = someM >>> fromM

optional ∷ ∀ f a. Alt f ⇒ Applicative f ⇒ f a → f (Maybe a)
...
instance parserResultShow ∷ Show a ⇒ Show (ParserResult a) where
  show = genericShow
...
instance argPolicyShow ∷ Show ArgPolicy where
  show = genericShow
...
instance optHelpInfoShow ∷ Show OptHelpInfo where
  show = genericShow
...
instance optTreeShow ∷ Show a ⇒ Show (OptTree a) where
  show = genericShow

optVisibility ∷ ∀ a. Option a → OptVisibility
optVisibility = _.propVisibility <<< un OptProperties <<< _.optProps <<<
  un Option

optHelp ∷ ∀ a. Option a → Chunk Doc
optHelp = _.propHelp <<< un OptProperties <<< _.optProps <<< un Option

optMetaVar ∷ ∀ a. Option a → String
optMetaVar = _.propMetaVar <<< un OptProperties <<< _.optProps <<< un
  Option

optShowDefault ∷ ∀ a. Option a → Maybe String
optShowDefault = _.propShowDefault <<< un OptProperties <<< _.optProps
  <<< un Option

optDescMod ∷ ∀ a. Option a → Maybe (Doc → Doc)
optDescMod = _.propDescMod <<< un OptProperties <<< _.optProps <<< un
  Option
