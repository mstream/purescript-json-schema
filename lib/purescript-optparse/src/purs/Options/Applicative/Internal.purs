...
import Control.Monad.Reader.Trans (ReaderT, ask, mapReaderT, runReaderT)
...
class (Monad m, Alt m) ⇐ MonadP (m ∷ Type → Type) where
  enterContext ∷ ∀ a. String → ParserInfo a → m Unit
  exitContext ∷ m Unit
  getPrefs ∷ m ParserPrefs

  missingArgP ∷ ∀ a. ParseError → Completer → m a
  errorP ∷ ∀ a. ParseError → m a
  exitP ∷ ∀ a b. IsCmdStart → ArgPolicy → Parser b → Maybe a → m a

newtype P a = P
  (ExceptT ParseError (StateT (Array Context) (Reader ParserPrefs)) a)
...
instance pMonadP ∷ MonadP P where
  enterContext name pinfo = P $ lift $ modify_ $ Array.(:) $ Context
    name
    (mkExists pinfo)
  ...
  exitP i _ p = P <<< maybe
    (throwError <<< MissingError i <<< SomeParser $ mkExists p)
    pure
  errorP = P <<< throwError

hoistMaybe ∷ ∀ a m. MonadP m ⇒ ParseError → Maybe a → m a
hoistMaybe err = maybe (errorP err) pure

hoistEither ∷ ∀ a m. MonadP m ⇒ Either ParseError a → m a
...
runP
  ...
  . P a
  → ParserPrefs
  → (Tuple (Either ParseError a) (Array Context))
...
runReadM ∷ ∀ a m. MonadP m ⇒ ReadM a → String → m a
runReadM (ReadM r) s = hoistEither <<< runExcept $ runReaderT r s

withReadM ∷ ∀ a. (String → String) → ReadM a → ReadM a
...
instance completionBind ∷ Bind Completion where
  bind (Completion x) k = Completion $ x >>= \a → case k a of
    Completion y → y
...
instance completionMonadP ∷ MonadP Completion where
  ...
  exitP _ a p _ = Completion <<< lift <<< lift $ ComplParser
    (SomeParser $ mkExists p)
    a
...
runCompletion
  ...
  . Completion r
  → ParserPrefs
  → Maybe (Either (Tuple SomeParser ArgPolicy) Completer)
runCompletion (Completion c) prefs =
  case runReaderT (runExceptT c) prefs of
...
stepListT ∷ ∀ m a. ListT m a → m (TStep a (ListT m a))
stepListT (ListT m) = m
...
bimapTStep ∷ ∀ a b x y. (a → b) → (x → y) → TStep a x → TStep b y
bimapTStep _ _ TNil = TNil
bimapTStep f g (TCons a x) = TCons (f a) (g x)

hoistList ∷ ∀ a m. Monad m ⇒ Array a → ListT m a
hoistList = Array.foldr (\x xt → ListT (pure (TCons x xt))) empty

takeListT ∷ ∀ m a. Monad m ⇒ Int → ListT m a → ListT m a
takeListT 0 = const empty
takeListT n = ListT <<< liftM1 (bimapTStep identity (takeListT (n - 1)))
  <<< stepListT

runListT ∷ ∀ a m. Monad m ⇒ ListT m a → m (List a)
runListT xs = do
  s ← stepListT xs
  case s of
    TNil → pure List.Nil
...
runNondetT ∷ ∀ m a. NondetT m a → ListT (StateT Boolean m) a
...
nondetTAltOp ∷ ∀ a m. Monad m ⇒ NondetT m a → NondetT m a → NondetT m a
nondetTAltOp m1 m2 = NondetT <<< alt (runNondetT m1) $ do
  s ← lift get
  guard (not s)
  runNondetT m2

cut ∷ ∀ m. Monad m ⇒ NondetT m Unit
cut = NondetT $ lift (put true)

disamb ∷ ∀ m a. Monad m ⇒ Boolean → NondetT m a → m (Maybe a)
