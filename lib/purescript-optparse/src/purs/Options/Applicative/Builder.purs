...
eitherReader ∷ ∀ a. (String → Either String a) → ReadM a
eitherReader f = readerAsk >>= either readerError pure <<< f

-- | Convert a function producing a 'Maybe' into a reader.
maybeReader ∷ ∀ a. (String → Maybe a) → ReadM a
maybeReader f = do
  arg ← readerAsk
  maybe (readerError $ "cannot parse value `" <> arg <> "'") pure <<< f
    $ arg
...
short ∷ ∀ f a. HasName f ⇒ Char → Mod f a
short = fieldMod <<< name <<< OptShort

-- | Specify a long name for an option.
long ∷ ∀ f a. HasName f ⇒ String → Mod f a
...
metavar ∷ ∀ f a. HasMetavar f ⇒ String → Mod f a
metavar var = optionMod $ over OptProperties \p → p
  { propMetaVar = var }
...
-- |        )
-- | ```
command ∷ ∀ a. String → ParserInfo a → Mod CommandFields a
...
commandGroup ∷ ∀ a. String → Mod CommandFields a
commandGroup g = fieldMod $ over CommandFields \p →
  p { cmdGroup = Just g }

-- | Add a list of possible completion values.
completeWith ∷ ∀ f a. HasCompleter f ⇒ Array String → Mod f a
...
-- | for a complete list.
action ∷ ∀ f a. HasCompleter f ⇒ String → Mod f a
...
completer ∷ ∀ f a. HasCompleter f ⇒ Completer → Mod f a
...
subparser ∷ ∀ a. Mod CommandFields a → Parser a
...
argument ∷ ∀ a. ReadM a → Mod ArgumentFields a → Parser a
...
flag' actv (Mod f d g) = mkParser d g rdr
  ...
  rdr =
    let
      (FlagFields fields) = f
        (FlagFields { flagNames: [], flagActive: actv })
    in
      FlagReader (fields.flagNames)
        (fields.flagActive)
...
-- | 'infoOption' instead.
abortOption
  ∷ ∀ a. ParseError → Mod OptionFields (a → a) → Parser (a → a)
...
infoOption ∷ ∀ a. String → Mod OptionFields (a → a) → Parser (a → a)
...
option ∷ ∀ a. ReadM a → Mod OptionFields a → Parser a
option r m = mkParser d g rdr
  where
  Mod f d g = metavar "ARG" `append` m
  (OptionFields fields) = f
    ( OptionFields
        { optNames: []
        , optCompleter: mempty
        , optNoArgError: ExpectsArgError
        }
    )
...
-- | value.
headerDoc ∷ ∀ a. Maybe Doc → InfoMod a
headerDoc doc = InfoMod $ over ParserInfo \i → i
  { infoHeader = Chunk doc }

-- | Specify a footer for this parser.
footer ∷ ∀ a. String → InfoMod a
...
-- | value.
footerDoc ∷ ∀ a. Maybe Doc → InfoMod a
footerDoc doc = InfoMod $ over ParserInfo \i → i
  { infoFooter = Chunk doc }

-- | Specify a short program description.
progDesc ∷ ∀ a. String → InfoMod a
progDesc s = InfoMod $ over ParserInfo \i → i
  { infoProgDesc = paragraph s }

-- | Specify a short program description as a 'Text.PrettyPrint.ANSI.Leijen.Doc'
-- | value.
progDescDoc ∷ ∀ a. Maybe Doc → InfoMod a
progDescDoc doc = InfoMod $ over ParserInfo \i → i
  { infoProgDesc = Chunk doc }

-- | Specify an exit code if a parse error occurs.
failureCode ∷ ∀ a. ExitCode → InfoMod a
...
noIntersperse = InfoMod $ over ParserInfo \p → p
...
forwardOptions ∷ ∀ a. InfoMod a
forwardOptions = InfoMod $ over ParserInfo \p → p
  { infoPolicy = ForwardOptions }

-- | Create a 'ParserInfo' given a 'Parser' and a modifier.
info ∷ ∀ a. Parser a → InfoMod a → ParserInfo a
...
multiSuffix s = PrefsMod $ over ParserPrefs \p → p
  { prefMultiSuffix = s }
...
disambiguate = PrefsMod $ over ParserPrefs \p → p
  { prefDisambiguate = true }

-- | Show full help text on any error.
showHelpOnError ∷ PrefsMod
showHelpOnError = PrefsMod $ over ParserPrefs \p → p
...
showHelpOnEmpty = PrefsMod $ over ParserPrefs \p → p
  { prefShowHelpOnEmpty = true }

-- | Turn off backtracking after subcommand is parsed.
noBacktrack ∷ PrefsMod
noBacktrack = PrefsMod $ over ParserPrefs \p → p
...
subparserInline = PrefsMod $ over ParserPrefs \p → p
  { prefBacktrack = SubparserInline }
