...
newtype ArgumentFields ∷ ∀ k. k → Type
...
class HasMetavar ∷ ∀ k. (k → Type) → Constraint
class HasMetavar f where
  hasMetavarDummy ∷ ∀ a. f a → Unit
...
optionMod ∷ ∀ f a. (OptProperties → OptProperties) → Mod f a
optionMod = Mod identity mempty

fieldMod ∷ ∀ f a. (f a → f a) → Mod f a
...
instance modSemigroup ∷ Semigroup (Mod f a) where
  append (Mod f1 d1 g1) (Mod f2 d2 g2) = Mod (f2 <<< f1) (d2 <> d1)
    (g2 <<< g1)
...
mkCommand
  ∷ ∀ a
  . Mod CommandFields a
  → Tuple3 (Maybe String) (Array String) (String → Maybe (ParserInfo a))
mkCommand m = group /\ map fst cmds /\ (_ `lookup` cmds) /\ unit
  where
  Mod f _ _ = m
  CommandFields { cmdCommands: cmds, cmdGroup: group } = f
    (CommandFields { cmdCommands: [], cmdGroup: Nothing })
...
mkProps (DefaultProp def sdef) g = props
  where
  props = over OptProperties (\r → r { propShowDefault = sdef <*> def })
    (g baseProps)

-- | Hide this option from the help text
internal ∷ ∀ f a. Mod f a
internal = optionMod $ over OptProperties \p → p
