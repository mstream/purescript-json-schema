...
optionNames ∷ ∀ a. OptReader a → Array OptName
...
optMatches disambiguate opt (OptWord arg1 val) = case opt of
  OptReader names rdr no_arg_err → do
    ...
    Just $ do
      args ← get
      let
        missing_arg = missingArgP (no_arg_err $ showOption arg1)
          (un CReader rdr).crCompleter
      Tuple arg' args' ← case maybe args (_ `List.Cons` args) val of
        List.Nil → lift missing_arg
        List.Cons arg' args' → pure $ Tuple arg' args'
      put args'
      lift $ runReadM
        (withReadM (errorFor arg1) (un CReader rdr).crReader)
        arg'
...
parseWord = toCharArray >>> List.fromFoldable >>> go
  ...
  go ('-' List.: '-' List.: w) = Just $
    ...
    in
      OptWord (OptLong (fromCharArray $ Array.fromFoldable opt))
        (fromCharArray <<< Array.fromFoldable <$> arg)
  go ('-' List.: w) = case w of
    List.Nil → Nothing
    (a List.: rest) → Just $
      let
...
searchParser
  ∷ ∀ m a
  . Monad m
  ⇒ (∀ r. Option r → NondetT m (Parser r))
  → Parser a
  → NondetT m (Parser a)
searchParser _ (NilP _) = empty
searchParser f (OptP opt) = f opt
searchParser f (MultP e) = runExists
  ( \(MultPE p1 p2) →
      let
...
searchParser f (BindP e) = e # Free.resume'
  ( \p k →
      oneOf
...
searchOpt pprefs w = searchParser \opt → do
  let
    disambiguate = (un ParserPrefs pprefs).prefDisambiguate &&
...
searchArg prefs arg = searchParser \opt → do
  ...
  case (un Option opt).optMain of
    CmdReader _ _ f →
      case (Tuple (f arg) (un ParserPrefs prefs).prefBacktrack) of
        (Tuple (Just subp) NoBacktrack) → lift $ do
          ...
          map pure <<< lift $ enterContext arg subp
            *> runParserInfo subp args
            <* exitContext

        (Tuple (Just subp) Backtrack) → map pure <<< lift <<< StateT $
          \args → do
            enterContext arg subp
              *> runParser (un ParserInfo subp).infoPolicy CmdStart
                (un ParserInfo subp).infoParser
                args
              <* exitContext
    ...
    ArgReader rdr →
      map pure <<< lift <<< lift $ runReadM (un CReader rdr).crReader
        arg
    _ → empty
...
runParser policy isCmdStart p args = case args of
  ...
  List.Cons "--" argt | policy /= AllPositionals → runParser
    AllPositionals
    CmdCont
    p
    argt
  ...
  newPolicy a = case policy of
    NoIntersperse →
      if isJust (parseWord a) then NoIntersperse else AllPositionals
    x → x

unexpectedError ∷ ∀ x. String → Parser x → ParseError
unexpectedError arg p = UnexpectedError arg $ SomeParser $ mkExists p

runParserInfo ∷ ∀ m a. MonadP m ⇒ ParserInfo a → Args → m a
runParserInfo i = runParserFully (un ParserInfo i).infoPolicy
  (un ParserInfo i).infoParser

runParserFully ∷ ∀ m a. MonadP m ⇒ ArgPolicy → Parser a → Args → m a
...
evalParser ∷ ∀ a. Parser a → Maybe a
evalParser (NilP r) = Just r
evalParser (OptP _) = Nothing
evalParser (MultP e) = runExists
  (\(MultPE p1 p2) → evalParser p1 <*> evalParser p2)
  e
evalParser (AltP p1 p2) = evalParser p1 <|> evalParser p2
evalParser (BindP e) = e # Free.resume'
  ( \p k →
...
treeMapParser g = simplify <<< go false false false g
  where
  has_default ∷ ∀ a'. Parser a' → Boolean
  ...
  go m d r f (OptP opt)
    | optVisibility opt > Internal = Leaf
        ( f
            ( OptHelpInfo
                { hinfoMulti: m
                , hinfoDefault: d
                , hinfoUnreachableArgs: r
                }
            )
            opt
        )
    | otherwise = MultNode []
  go m d r f (MultP e) = runExists
    ( \(MultPE p1 p2) →
        let
          r' = r || hasArg p1
        in
  ...
  go _ d r f (BindP e) = e # Free.resume'
    ( \p k →
        let
          go' = go true d r f p
        in
