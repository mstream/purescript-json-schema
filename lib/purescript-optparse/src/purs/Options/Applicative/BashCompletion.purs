...
instance richnessShow ∷ Show Richness where
  show = genericShow

bashCompletionParser
  ∷ ∀ a. ParserInfo a → ParserPrefs → Parser CompletionResult
  ...
  complParser =
    ( failure <$>
        ( bashCompletionQuery pinfo pprefs
            ...
            <$>
              ( flag' Enriched
                  (long "bash-completion-enriched" `append` internal)
                  <*> option int
                    ( long "bash-completion-option-desc-length"
                        `append` internal
                        `append` value 40
                    )
                  <*> option int
                    ( long "bash-completion-command-desc-length"
                        `append` internal
                        `append` value 40
                    )
                  <|> pure Standard
              )
            <*> (map Array.fromFoldable <<< many <<< strOption)
              ( long "bash-completion-word"
                  `append` internal
              )
            <*> option int
              (long "bash-completion-index" `append` internal)
        )
    )
      <|> failure <$>
...
bashCompletionQuery
  ...
  → String
  → Effect (Array String)
bashCompletionQuery pinfo pprefs richness ws i _ =
  case runCompletion compl pprefs of
  ...
  list_options ∷ ∀ x. ArgPolicy → Parser x → Effect (Array String)
  list_options a = map fold
    <<< sequence
  ...
  opt_completions
    ∷ ∀ x. ArgPolicy → OptHelpInfo → Option x → Effect (Array String)
  opt_completions argPolicy hinfo opt = case (un Option opt).optMain of
    OptReader ns _ _
      | argPolicy /= AllPositionals → pure <<< add_opt_help opt $
          show_names ns
      | otherwise → pure []
    FlagReader ns _
      | argPolicy /= AllPositionals → pure <<< add_opt_help opt $
      ...
      | otherwise → pure []
    ArgReader rdr
      | (un OptHelpInfo hinfo).hinfoUnreachableArgs → pure []
      | otherwise → run_completer (un CReader rdr).crCompleter
    CmdReader _ ns p
      | (un OptHelpInfo hinfo).hinfoUnreachableArgs → pure []
      | otherwise → pure <<< add_cmd_help p $ filter_names ns

  -- When doing enriched completions, add any help specified
  -- to the completion variables (tab separated).
  add_opt_help ∷ ∀ x f. Functor f ⇒ Option x → f String → f String
  add_opt_help opt = case richness of
    ...
    → map
      ( \o →
          let
            h = un Chunk $ optHelp opt
          in
            maybe o (\h' → o <> "\t" <> render_line len h') h
      )
  ...
  add_cmd_help p = case richness of
    ...
    → map
      ( \cmd →
          let
            h = p cmd >>= un Chunk <<< _.infoProgDesc <<< un ParserInfo
          in
            maybe cmd (\h' → cmd <> "\t" <> render_line len h') h
      )
  ...
  render_line len doc =
    case
      map NEA.uncons $ NEA.fromArray $ lines
        (displayS (renderPretty 1.0 len doc))
      of
      Nothing → ""
      Just { head: x, tail: [] } → x
...
arraySplitAt idx arr =
  case idx of
    0 →
      { init: [], rest: arr }
    i →
      { init: Array.slice 0 i arr
      , rest: Array.slice i (Array.length arr) arr
      }
...
fishCompletionScript prog progn = pure
  ...
  , "end"
  , ""
  , "complete --no-files --command " <> progn <> " --arguments '(_"
      <> progn
      <> ")'"
  ]
