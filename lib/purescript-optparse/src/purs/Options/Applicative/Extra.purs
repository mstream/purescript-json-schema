...
exitSuccess ∷ ∀ t270. Effect t270
exitSuccess = exit' $ fromEnum ExitCode.Success

exitWith ∷ ∀ void. ExitCode → Effect void
...
-- | the subcommand.
hsubparser ∷ ∀ a. Mod CommandFields a → Parser a
...
-- | occurs.
execParser ∷ ∀ a. ParserInfo a → Effect a
execParser = customExecParser defaultPrefs

-- | Run a program description with custom preferences.
customExecParser ∷ ∀ a. ParserPrefs → ParserInfo a → Effect a
customExecParser pprefs pinfo = execParserPure pprefs pinfo <$> getArgs
  >>= handleParseResult

-- | Handle `ParserResult`.
handleParseResult ∷ ∀ a. ParserResult a → Effect a
...
getParseResult ∷ ∀ a. ParserResult a → Maybe a
...
parserFailure pprefs pinfo msg ctx = ParserFailure $ \progn →
  ...
  error_help = errorHelp $ case msg of
    ...
    → runExists (\x → stringChunk "Missing:" <<+>> missingDesc pprefs x)
      e
    ...
    →
      let
        ...
        -- reported
        msg' =
          if startsWith (String.Pattern "-") arg then "Invalid option `"
            <> arg
            <> "'"
  ...
  suggestion_help = suggestionsHelp $ case msg of
    ...
    →
      let
        ...
        possibles = fold $ runExists
          (\zz → mapParser opt_completions zz)
          x
        ...
        opt_completions ∷ ∀ g. OptHelpInfo → Option g → Array String
        opt_completions (OptHelpInfo hinfo) (Option opt) =
          case opt.optMain of
  ...
  base_help ∷ ∀ x. ParserInfo x → ParserHelp
  ...
  show_full_help = case msg of
    ShowHelpText → true
    MissingError CmdStart _
      | (un ParserPrefs pprefs).prefShowHelpOnEmpty → true
    _ → (un ParserPrefs pprefs).prefShowHelpOnError

renderFailure
  ∷ ParserFailure ParserHelp → String → Tuple String ExitCode
