...
optDesc
  ∷ ∀ a. ParserPrefs → OptDescStyle → OptHelpInfo → Option a → Chunk Doc
optDesc pprefs style info opt =
  let
    ns = optionNames (un Option opt).optMain
    mv = stringChunk $ optMetaVar opt
    descs = map (string <<< showOption) (Array.sort ns)
    desc' =
      listToChunk (intersperse (un OptDescStyle style).descSep descs)
        <<+>> mv
    show_opt
      | (un OptHelpInfo info).hinfoDefault && not
          (un OptDescStyle style).descOptional = false
      | optVisibility opt == Hidden = (un OptDescStyle style).descHidden
      | otherwise = optVisibility opt == Visible
    suffix
      | (un OptHelpInfo info).hinfoMulti = stringChunk $
          (un ParserPrefs pprefs).prefMultiSuffix
      | otherwise = mempty
    render chunk
      | not show_opt = mempty
      | isEmpty chunk || not (un OptDescStyle style).descSurround =
          append chunk suffix
      | (un OptHelpInfo info).hinfoDefault = append (map brackets chunk)
          suffix
      | Array.null (Array.drop 1 descs) = append chunk suffix
      | otherwise = append (map parens chunk) suffix
  in
    maybe identity map (optDescMod opt) (render desc')

intersperse ∷ ∀ a. a → Array a → Array a
intersperse sep = Array.fold <<< Array.mapWithIndex \idx e →
  if idx == 0 then [ e ] else [ sep, e ]

-- | Generate descriptions for commands.
cmdDesc ∷ ∀ a. Parser a → Array (Tuple (Maybe String) (Chunk Doc))
cmdDesc = mapParser desc
  where
  desc ∷ ∀ x. OptHelpInfo → Option x → Tuple (Maybe String) (Chunk Doc)
  desc _ opt =
    case (un Option opt).optMain of
      CmdReader gn cmds p → Tuple gn $
        tabulate do
          cmd ← Array.reverse cmds
          d ← maybe [] pure <<< map (un ParserInfo >>> _.infoProgDesc) $
            p cmd
          pure $ Tuple (string cmd) (align (extractChunk d))
      _ → mempty

-- | Generate a brief help text for a parser.
briefDesc ∷ ∀ a. ParserPrefs → Parser a → Chunk Doc
briefDesc = briefDesc' true

-- | Generate a brief help text for a parser, only including mandatory
--   options and arguments.
missingDesc ∷ ∀ a. ParserPrefs → Parser a → Chunk Doc
...
briefDesc' ∷ ∀ a. Boolean → ParserPrefs → Parser a → Chunk Doc
briefDesc' showOptional pprefs = fold_tree <<< treeMapParser
  (optDesc pprefs style)
  where
  style = OptDescStyle
    { descSep: string "|"
    , descHidden: false
...
fullDesc ∷ ∀ a. ParserPrefs → Parser a → Chunk Doc
fullDesc pprefs = tabulate <<< Array.catMaybes <<< mapParser doc
  where
  doc ∷ ∀ x. OptHelpInfo → Option x → Maybe (Tuple Doc Doc)
  doc info opt = do
    guard $ not $ isEmpty n
    guard $ not $ isEmpty h
...
suggestionsHelp chunk = over ParserHelp _ { helpSuggestions = chunk }
  mempty
...
parserHelp ∷ ∀ a. ParserPrefs → Parser a → ParserHelp
parserHelp pprefs p = bodyHelp <<< vsepChunks $
  ...
  group_title arr =
    let
      { head, tail } = (NEA.uncons arr)
    in
      with_title (fromMaybe def $ fst head) $ vcatChunks
        ([ snd head ] <> (snd <$> tail))
...
parserUsage ∷ ∀ a. ParserPrefs → Parser a → String → Doc
