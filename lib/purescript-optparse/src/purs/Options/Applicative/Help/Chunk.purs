...
instance chunkShow ∷ Show a ⇒ Show (Chunk a) where
  show = genericShow
...
listToChunk ∷ ∀ a. Monoid a ⇒ Array a → Chunk a
listToChunk [] = mempty
...
extractChunk ∷ ∀ a. Monoid a ⇒ Chunk a → a
...
tabulate' _ [] = mempty
tabulate' size table = pure $ vcat $ table <#> \(Tuple key value) →
