...
run ∷ ∀ a. ParserInfo a → Array String → ParserResult a
...
assertError
  ...
  . Show a
  ⇒ ParserResult a
  → (ParserFailure ParserHelp → QC.Result)
  → QC.Result
assertError x f = case x of
  Success r → counterExample
    ("expected failure, got success: " <> show r)
  Failure e → f e
  CompletionInvoked _ → counterExample
    "expected failure, got completion"

assertResult ∷ ∀ a. ParserResult a → (a → QC.Result) → QC.Result
...
assertHasLine l s = l `elem` lines s <?>
  ("expected line:\n\t" <> l <> "\nnot found")
...
mkAsset name msg =
  { name
  , msg: stripNl String.stripPrefix $ stripNl String.stripSuffix $ msg
  }
  where
...
checkHelpTextWith ecode pprefs asset p args =
  let
    result = execParserPure pprefs p args
  in
    assertError result $ \failure → Tuple asset.msg ecode ===
      renderFailure failure asset.name

checkHelpText
  ∷ ∀ a. Show a ⇒ Asset → ParserInfo a → Array String → QC.Result
...
spec = describe "optparse" $ do
  ...
  it "prop_cmd_header" $ defer \_ → do
    ...
    isOK $ conjoin
      [ checkHelpTextWith ExitCode.Error defaultPrefs
          asset_commands_header
          i
          [ "-zello" ]
      , checkHelpTextWith ExitCode.Error (prefs showHelpOnError)
          asset_commands_header_full
          i
          [ "-zello" ]
      ]

  it "prop_cabal_conf" $ defer \_ → do
    isOK $ checkHelpText asset_cabal Cabal.pinfo
      [ "configure", "--help" ]
  ...
  it "prop_args_ddash" $ defer \_ → do
    let
      result = run Commands.opts
        [ "hello", "foo", "--", "--bar", "--", "baz" ]
    isOK $ assertResult result
      (_ === (Commands.Hello [ "foo", "--bar", "--", "baz" ]))

  it "prop_alts" $ defer \_ → do
    let
      result = run Alternatives.opts
        [ "-b", "-a", "-b", "-a", "-a", "-b" ]
  ...
  it "prop_alt_cont" $ defer \_ → do
    let
      p = Alternatives.a <|> Alternatives.b
      i = info p mempty
      result = run i [ "-a", "-b" ]
    isOK $ assertError result $ const QC.Success
  ...
  it "prop_nested_commands" $ defer \_ → do
    ...
    isOK $ checkHelpTextWith ExitCode.Error defaultPrefs asset_nested i
      [ "c", "b" ]
  ...
  it "prop_drops_back_contexts" $ defer \_ → do
    ...
    isOK $ checkHelpTextWith ExitCode.Error defaultPrefs asset_dropback
      i
      [ "b", "-aA" ]
  ...
  it "prop_context_carry" $ defer \_ → do
    let
      ...
      p0 = Tuple <$> p2 <*> p1
      i = info (p0 <**> helper) mempty
    isOK $ checkHelpTextWith ExitCode.Error defaultPrefs asset_carry i
      [ "b", "-aA", "c" ]
  ...
  it "prop_help_on_empty" $ defer \_ → do
    let
      ...
      p0 = Tuple <$> p2 <*> p1
      i = info (p0 <**> helper) mempty
    isOK $ checkHelpTextWith ExitCode.Error (prefs showHelpOnEmpty)
      asset_helponempty
      i
      []
  ...
  it "prop_help_on_empty_sub" $ defer \_ → do
    let
      p3 = strOption
        (short 'a' <> metavar "A" <> help "both commands require this")
      p2 = subparser (command "b" (info p3 mempty) <> metavar "B")
      p1 = subparser (command "c" (info p3 mempty) <> metavar "C")
      p0 = Tuple <$> p2 <*> p1
      i = info (p0 <**> helper) mempty
    isOK $ checkHelpTextWith ExitCode.Error (prefs showHelpOnEmpty)
      asset_helponemptysub
      i
      [ "b", "-aA", "c" ]
  ...
  it "prop_completion_rich" $ defer \_ → do
    let
      ...
      i = info p mempty
      result = run i
        [ "--bash-completion-enriched", "--bash-completion-index", "0" ]
    isOK =<< assertCompletion result
      ([ "--foo\tFo?", "--bar\tBa?" ] === _)
  ...
  it "prop_bind_usage" $ defer \_ → do
    let
      p = Array.fromFoldable <$> many (argument str (metavar "ARGS..."))
      i = info (p <**> helper) briefDesc
      result = run i [ "--help" ]
    isOK $ assertError result $ \failure →
      let
        text = Array.head $ lines $ fst $ renderFailure failure "test"
      in
        Just "Usage: test [ARGS...]" === text

  it "prop_issue_19" $ defer \_ → do
    let
      p = option (map Just str) (short 'x' <> value Nothing)
      i = info (p <**> helper) mempty
      result = run i [ "-x", "foo" ]
  ...
  it "prop_arguments1_none" $ defer \_ → do
    let
      p = Array.fromFoldable <$> some (argument str mempty)
      i = info (p <**> helper) mempty
      result = run i []
    isOK $ assertError result $ const QC.Success
  ...
  it "prop_issue_35" $ defer \_ → do
    let
      p = flag' true (short 't' <> hidden) <|> flag' false (short 'f')
      i = info p mempty
      result = run i []
    isOK $ assertError result $ \failure →
      let
        text = lines <<< fst $ renderFailure failure "test"
      in
  ...
  it "prop_error_context" $ defer \_ → do
    ...
    isOK $ assertError result $ \failure →
      ...
      in
        conjoin
          [ String.Pattern "port" `isInfixOf` errMsg <?>
              "no context in error message (option)"
          , String.Pattern "foo" `isInfixOf` errMsg <?>
              "no context in error message (value)"
          ]
  ...
  it "prop_unix_style" $ quickCheck \j k →
    let
      ...
      result = run i [ "-xc" ]
    in
      assertResult result (_ === (Tuple j k))
  ...
  it "prop_count_flags" $ defer \_ → do
    let
      p = List.length <$> many (flag' unit (short 't'))
      i = info p mempty
      result = run i [ "-ttt" ]
  ...
  it "prop_issue_47" $ defer \_ → do
    let
      p = option r (long "test" <> value 9)
      r = readerError "error message"
      result = run (info p mempty) [ "--test", "x" ]
    isOK $ assertError result $ \failure →
      let
        text = fromMaybe "" $ Array.head $ lines $ fst $ renderFailure
          failure
          "test"
      in
        String.Pattern "error message" `isInfixOf` text <?>
          "no error message"

  it "prop_long_help" $ defer \_ → do
    let
      p = Formatting.opts <**> helper
      i = info p
        ( progDesc
            ( fold
    ...
    isOK $ checkHelpTextWith ExitCode.Success (prefs (columns 50))
      asset_formatting
      i
      [ "--help" ]
  ...
  it "prop_intersperse_1" $ defer \_ → do
    let
      ...
      result = run (info p noIntersperse)
        [ "a", "-x", "b" ]
    isOK $ assertResult result $ \args → [ "a", "-x", "b" ] ===
      Array.fromFoldable args
  ...
  it "prop_intersperse_2" $ defer \_ → do
    ...
    isOK $ conjoin
      [ assertResult result1 $ \args → [ "foo", "-x" ] ===
          Array.fromFoldable args
    ...
    isOK $ assertError (run i [ "-a" ]) $ \failure →
      let
        text = fromMaybe "" $ Array.head $ lines $ fst $ renderFailure
          ...
          "test"
      in
        "The option `-a` expects an argument." === text

  it "prop_many_pairs_success" $ defer \_ → do
    let
      p = many $ Tuple <$> argument str mempty <*> argument str mempty
      i = info p mempty
  ...
  it "prop_many_pairs_lazy_progress" $ defer \_ → do
    let
      p = many $ Tuple <$> optional (option str (short 'a')) <*>
        argument str mempty
      i = info p mempty
      result = run i [ "foo", "-abar", "baz" ]
    isOK $ assertResult result $ \xs →
      [ (Tuple (Just "bar") "foo"), (Tuple Nothing "baz") ] ===
        Array.fromFoldable xs

  it "prop_suggest" $ defer \_ → do
    let
      p2 = subparser (command "first" (info (pure unit) mempty))
    ...
    isOK $ assertError result $ \failure →
      let
        (Tuple msg _) = renderFailure failure "prog"
      in
        String.Pattern "Did you mean one of these?\n    first\n    fst"
          `isInfixOf` msg <?> msg
...
prop_paragraph s = isEmpty (paragraph s) == Array.null (words s) <?>
  show { s, p: paragraph s, w: words s }
...
prop_edit_distance_gezero a b =
  editDistance (toCharArray a) (toCharArray b) >= 0
...
prop_edit_substitution ∷ ∀ m. MonadGen m ⇒ MonadRec m ⇒ m QC.Result
...
prop_edit_transposition ∷ ∀ m. MonadGen m ⇒ MonadRec m ⇒ m QC.Result
...
quickCheckGen ∷ ∀ p. QC.Testable p ⇒ Gen p → Aff Unit
...
quickCheckPure seed n prop = do
  ...
  if List.length msgs > 0 then throwError $ error $ List.intercalate
    "\n  "
    msgs
  else pure unit
